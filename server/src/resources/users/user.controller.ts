...
const createInitialUserData = async (userId:number, name:string):Promise<void> =>{
  ...
  const profile = {
    user_id:userId, 'nickName':name, level:INITIAL_LEVEL, exp:INITIAL_EXP, curr_user_deck_id:deckId};
...
const updateUserProfile = async (id:number, data:UserProfile):Promise<UserProfile> => usersModel.updateUserProfile(id, data);

const updateDefaultDeck =  async (user_id:number, deck_id:number):Promise<number> => usersModel.updateDefaultDeck(user_id, deck_id);
